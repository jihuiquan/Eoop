...
public class PopupList implements View.OnTouchListener {
    ...
    public void showPopupWindow(final Context context, View parent, int position, List popupList, float rawX, float rawY) {
        ...
        //计算出弹出窗口的宽高
        int PopupWindowWidth = recyclerView.getAdapter().getItemCount() * ScreenUtils.dp2px(mContext,itemViewWidth) +
                (recyclerView.getAdapter().getItemCount() - 1) * ScreenUtils.dp2px(mContext,dividerWidth);
        int PopupWindowHeight = ScreenUtils.dp2px(mContext,itemViewHeight + arrowHeight);
        ...
        if (leftEdgeOffset < PopupWindowWidth / 2) {
            if (leftEdgeOffset < ScreenUtils.dp2px(mContext,arrowWidth / 2.0f)) {
                iv.setTranslationX(ScreenUtils.dp2px(mContext,windowCornersRadius));
            } else {
                iv.setTranslationX(leftEdgeOffset - ScreenUtils.dp2px(mContext,arrowWidth / 2.0f));
            }
        } else if (rightEdgeOffset < PopupWindowWidth / 2) {
            if (rightEdgeOffset < ScreenUtils.dp2px(mContext,arrowWidth / 2.0f)) {
                iv.setTranslationX(PopupWindowWidth - rightEdgeOffset - ScreenUtils.dp2px(mContext,arrowWidth / 2.0f)-ScreenUtils.dp2px(mContext,windowCornersRadius));
            } else {
                iv.setTranslationX(PopupWindowWidth - rightEdgeOffset - ScreenUtils.dp2px(mContext,arrowWidth / 2.0f));
            }
        } else {
            iv.setTranslationX(PopupWindowWidth / 2 - ScreenUtils.dp2px(mContext,arrowWidth / 2.0f));
        }
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(ScreenUtils.dp2px(mContext,arrowWidth), ScreenUtils.dp2px(mContext,arrowHeight));
        ...
        popupListWindow.showAtLocation(parent, Gravity.NO_GRAVITY, (location[0] + parent.getWidth() / 2) - PopupWindowWidth / 2,
     ...
     */
    public void initPopupList(final Context context,View view,int position, final AbsListView absListView, final List<String> popupMenuItemList, PopupListAdapter.OnPopupListClickListener popupListOnClickListener) {
