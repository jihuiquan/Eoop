...
//		}
	...
	public Bitmap clip() {
		...
		//TODO 尺寸需要判断
        if((getWidth() - borderlength) / 2 >=0 && (getHeight() - borderlength) / 2>=0){
            return Bitmap.createBitmap(bitmap, (getWidth() - borderlength) / 2,
                    (getHeight() - borderlength) / 2, borderlength, borderlength);
        }else if((getWidth() - borderlength) / 2 <0 && (getHeight() - borderlength) / 2>=0){
			return Bitmap.createBitmap(bitmap, getWidth(),
					(getHeight() - borderlength) / 2, borderlength, borderlength);
        }else if((getWidth() - borderlength) / 2 >=0 && (getHeight() - borderlength) / 2<0){
			return Bitmap.createBitmap(bitmap, (getWidth() - borderlength) / 2,
					getHeight(), borderlength, borderlength);
		}else{
			return bitmap;
		}
