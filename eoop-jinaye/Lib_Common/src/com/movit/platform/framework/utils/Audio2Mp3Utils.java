...
public class Audio2Mp3Utils {
	 ...
	 * @param activity
	 */
	public Audio2Mp3Utils(Activity activity) {
	 ...
	 * @param mp3Path
	 */
	public Audio2Mp3Utils(Activity activity, String rawPath,
						  String mp3Path) {
		this.activity = activity;
	...
	public boolean startRecording() {
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			cleanFile(Audio2Mp3Utils.RAW | Audio2Mp3Utils.MP3);
			isRecording = false;
			return isRecording;
		}
			...
			public void run() {
				...
				try {
					...
					while (isRecording) {
						...
						valueEMA = (int) (Math
								.abs((int) (v / (float) readSize) / 10000) >> 1);
						dB = Math.log10(v / (double) readSize);
					}
	 ...
	 */
	class Sample {
		Audio2Mp3Utils util = null;
		String mp3 = null;
		String raw = null;

		void start() {
			// util=new Audio2Mp3Utils(activity); 一般是用这个
			if (util == null)
				util = new Audio2Mp3Utils(null, "/sdcard/sample.raw",
						"/sdcard/sample.mp3");
