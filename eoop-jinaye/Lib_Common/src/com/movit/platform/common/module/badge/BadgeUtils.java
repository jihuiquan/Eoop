package com.movit.platform.common.module.badge;
...
public class BadgeUtils {
     ...
     */
    public static void setBadgeNum(Context context, Class<?> clazz, boolean isShowNum, String num, boolean isStroke) {
        if (isShowNum)
            return;
        ...
        if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi")) {
            ...
            htcIntent.putExtra("packagename", context.getPackageName());
            htcIntent.putExtra("count", num);
            context.sendBroadcast(htcIntent);
        }else if (Build.MANUFACTURER.equalsIgnoreCase("lg")) {
            //LG
            Intent lgIntent = new Intent("android.intent.action.BADGE_COUNT_UPDATE");
            lgIntent.putExtra("badge_count_package_name", context.getPackageName());
            lgIntent.putExtra("badge_count_class_name", getLaunchActivityName(context));
            lgIntent.putExtra("badge_count", num);
            context.sendBroadcast(lgIntent);
        }else {//其他原生系统手机
    ...
    public static void xiaoMiShortCut(Context context, Class<?> clazz, String num) {
        ...
        try {
            ...
            field.setAccessible(true);
            field.set(notification, miuiNotification); // 设置信息数-->这种发送必须是miui 6才行
    ...
    private static ShapeDrawable getDefaultBackground2(Context context) {
        ...
        //圆角矩形
        RoundRectShape rr = new RoundRectShape(outerR, null, null);
        CBorderDrawable drawable = new CBorderDrawable(context, rr);
