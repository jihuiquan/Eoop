...
public class FFmpegRecorderActivity extends Activity implements OnClickListener, OnTouchListener {
    ...
    //提示换个场景
    public static int recordingChangeTime = 1000;
...
//			}
            ...
            media.setDataSource(strVideoPath);
            Bitmap bitmap = media.getFrameAtTime();
            File myCaptureFile = new File(captureBitmapPath);
...
//				getFirstCapture(firstData);
    ...
    private void returnToCaller(boolean valid) {
        recordSuccess = valid;
        try {
            setActivityResult(valid, strVideoPath, imagePath);
    ...
    private void setActivityResult(boolean valid, String strVideoPath, String imagePath) {
        Intent resultIntent = new Intent();
        int resultCode;
        if (valid) {
            resultCode = RESULT_OK;
//			resultIntent.setData(uriVideoPath);
            resultIntent.putExtra("videoPath", strVideoPath);
            resultIntent.putExtra("firstImagePath", imagePath);
        } else {
