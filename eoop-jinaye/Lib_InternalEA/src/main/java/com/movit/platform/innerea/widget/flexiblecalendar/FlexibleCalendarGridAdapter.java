...
class FlexibleCalendarGridAdapter extends BaseAdapter {
	...
	@Override
	public int getCount() {
		return showDatesOutsideMonth ? SIX_WEEK_DAY_COUNT : monthDisplayHelper
				.getNumberOfDaysInMonth()
				+ ((monthDisplayHelper.getFirstDayOfMonth() - 1 + 6) % 7);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		boolean isWithinCurrentMonth = monthDisplayHelper.isWithinCurrentMonth(
				row, col);

		BaseCellView cellView = cellViewDrawer.getCellView(position,
				convertView, parent, isWithinCurrentMonth);
		if (cellView == null) {
	...
	private void drawDateCell(BaseCellView cellView, int position,
			boolean isWithinCurrentMonth, int row, int col) {
		if (isWithinCurrentMonth) {
			...
			if (monthEventFetcher != null) {
				if (cellView.getEvents() == null) {
					cellView.setEvents(monthEventFetcher.getEventsForTheDay(
							year, month, day));
				}
			...
			if (showDatesOutsideMonth) {
				...
				cellView.setText(String.valueOf(day));
				int[] temp = new int[2];
				// date outside month and less than equal to 12 means it belongs
				// to next month otherwise previous
				if (day <= 12) {
					FlexibleCalendarHelper.nextMonth(year, month, temp);
					cellView.setOnClickListener(new DateClickListener(day,
							temp[1], temp[0], position));
				} else {
	...
	public void setOnDateClickListener(
			OnDateCellItemClickListener onDateCellItemClickListener) {
